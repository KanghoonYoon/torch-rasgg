...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0, dataset_to_test=None):
    assert mode in {'train', 'val', 'test', 'extract_train', 'extract_val', 'extract_test'}
    ...
    is_train = (mode == 'train') | (mode == 'extract_train') | (mode == 'extract_val') | (mode == 'extract_test')
    if mode == 'train':
    ...
    DatasetCatalog = paths_catalog.DatasetCatalog
    if (dataset_to_test == 'train') | (dataset_to_test == 'extract_train'):
        dataset_list = cfg.DATASETS.TRAIN
    elif (dataset_to_test == 'val') | (dataset_to_test == 'extract_val'):
    ...
    if (mode == 'extract_train') | (mode == 'extract_val') | (mode == 'extract_test'):
        if cfg.GLOBAL.DATASET_NAME == 'gqa':
            num_iters = int(len(datasets[0].filenames) / cfg.SOLVER.IMS_PER_BATCH)
        else:
            num_iters = int(datasets[0].split_mask.sum() / cfg.SOLVER.IMS_PER_BATCH)
        start_iter = 0

    print("NumIters:", num_iters)
