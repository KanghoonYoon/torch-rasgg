...
class RelationLossComputation(object):
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, rel_loss_weight=None, logit_adjust=False):
        """
        Computes the loss for relation triplet.
        ...
        if self.use_label_smoothing:
            loss_relation = Label_Smoothing_Regression(e=0.01)(relation_logits, rel_labels.long())

        elif self.cfg.RASGG.MIXUP:
            loss_relation = F.cross_entropy(relation_logits, rel_labels, weight=rel_loss_weight)
        else:
            loss_relation = F.cross_entropy(relation_logits, rel_labels.long(), weight=rel_loss_weight)  
...
def make_roi_relation_loss_evaluator(cfg):
    ...
    loss_evaluator = RelationLossComputation(
        ...
        cfg.MODEL.ROI_RELATION_HEAD.REL_PROP, cfg
    )
