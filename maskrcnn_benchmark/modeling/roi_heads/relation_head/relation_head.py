...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None, cur_iter=None):
        ...
        elif (not self.training) & (self.cfg.TYPE == 'ra_extract'):
            proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_relsample(proposals, targets)
        else:
        ...
        refine_logits, relation_logits, add_losses, add_data = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger, cur_iter)
        ...
        if ('ReTAG' in self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR):
            rel_labels = add_data['rel_labels']

        loss_relation, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits, refine_logits, rel_loss_weight=self.predictor.rel_loss_weight, logit_adjust=None)
    ...
    def predict_and_retrieve(self, features, proposals, targets=None, logger=None, cur_iter=None):
        """
        Arguments:
            features (list[Tensor]): feature-maps from possibly several levels
            proposals (list[BoxList]): proposal boxes. Note: it has been post-processed (regression, nms) in sgdet mode
        ...
        # final classifier that converts the features into predictions
        # should corresponding to all the functions and layers after the self.context class
        refine_logits, relation_logits, add_losses, add_data, retrieved_value = self.predictor.predict_and_retrieve(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger, cur_iter)
        ...
        if retrieved_value is not None:
            ...
            for i, boxlist in enumerate(result):
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    boxlist.add_field('retrieved_rel_labels', np.array(retrieved_value))
                result[i] = boxlist
