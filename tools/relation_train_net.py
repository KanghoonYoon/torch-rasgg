...
def train(cfg, local_rank, distributed, logger):
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        if iteration % 500 == 0:
            try:
                memory_size = sum(model.roi_heads.relation.predictor.featurebank_counter > 100) * 100 + sum(model.roi_heads.relation.predictor.featurebank_counter[(model.roi_heads.relation.predictor.featurebank_counter < 100)])
                logger.info('Memory Bank size: {}'.format(memory_size))
            except:
                pass
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/e2e_relation_X_101_32_8_FPN_1x_rasgg.yaml",
        metavar="FILE",
